...
1. **Clé de matching persona** — `adresse.personas` contient les **libellés de lien** Baserow (`linkValues` renvoie le champ primaire de la table Personas). Le moteur compare avec `clePersona(persona)` : il faut que ce soit la **même clé des deux côtés** (le `code` 01-08, ou le `nom_fr`). À fixer pour éviter un match qui échoue silencieusement. *Recommandation : utiliser `code` comme champ primaire des Personas.*
